...
async function writeState(ctx, state) {
  await Promise.all([
    ...
    ctx.state.set({ scopeKind: "instance", stateKey: STATE_KEYS.pausedAt }, state.pausedAt).catch(() => null),
    ctx.state.set({ scopeKind: "instance", stateKey: STATE_KEYS.pausedBy }, state.pausedBy).catch(() => null),
    ctx.state.set({ scopeKind: "instance", stateKey: STATE_KEYS.pausedReason }, state.pausedReason).catch(() => null),
    ctx.state.set({ scopeKind: "instance", stateKey: STATE_KEYS.summaryAgents }, state.summary.agents).catch(() => null),
    ctx.state.set({ scopeKind: "instance", stateKey: STATE_KEYS.summaryCompanies }, state.summary.companies).catch(() => null),
    ctx.state.set({ scopeKind: "instance", stateKey: STATE_KEYS.summaryAt }, state.summary.computedAt).catch(() => null)
  ]);
}
var pluginInstance = definePlugin({
  async setup(ctx) {
    ...
    ctx.actions.register(ACTION_KEYS.setPaused, async (params) => {
      const p = params ?? {};
      ...
      const next = {
        paused,
        pausedAt: paused ? (/* @__PURE__ */ new Date()).toISOString() : null,
    ...
    ctx.actions.register(ACTION_KEYS.refreshSummary, async (params) => {
      const p = params ?? {};
