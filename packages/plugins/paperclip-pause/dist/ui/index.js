// src/ui/index.tsx
import { useCallback, useEffect, useMemo, useState } from "react";
import {
  usePluginData,
  usePluginAction,
  useHostContext
} from "@paperclipai/plugin-sdk/ui";
import { jsx, jsxs } from "react/jsx-runtime";
var DEFAULT_STATE = {
  paused: false,
  pausedAt: null,
  pausedBy: null,
  pausedReason: null,
  summary: { agents: 0, companies: 0, computedAt: null }
};
async function postAgentPause(agentId, paused) {
  const path = `/api/agents/${encodeURIComponent(agentId)}/${paused ? "pause" : "resume"}`;
  return fetch(path, {
    method: "POST",
    credentials: "include",
    headers: { "Content-Type": "application/json", Accept: "application/json" },
    body: "{}"
  });
}
async function listAgentsForCompany(companyId) {
  const r = await fetch(
    `/api/companies/${encodeURIComponent(companyId)}/agents?limit=500`,
    { credentials: "include", headers: { Accept: "application/json" } }
  );
  if (!r.ok) return [];
  const json = await r.json();
  return Array.isArray(json.items) ? json.items : [];
}
function formatRelative(iso) {
  if (!iso) return "";
  const then = new Date(iso).getTime();
  if (Number.isNaN(then)) return "";
  const secs = Math.max(0, Math.round((Date.now() - then) / 1e3));
  if (secs < 60) return `${secs}s ago`;
  if (secs < 3600) return `${Math.round(secs / 60)}m ago`;
  return `${Math.round(secs / 3600)}h ago`;
}
function PauseRunButton({ context }) {
  const host = useHostContext();
  const companyId = host.companyId ?? context.companyId ?? null;
  const [pollTick, setPollTick] = useState(0);
  useEffect(() => {
    const t = setInterval(() => setPollTick((n) => n + 1), 5e3);
    return () => clearInterval(t);
  }, []);
  const stateResult = usePluginData("state", { _tick: pollTick });
  const setPaused = usePluginAction("set-paused");
  const refreshSummary = usePluginAction("refresh-summary");
  const state = useMemo(() => {
    const data = stateResult && stateResult.data;
    return data && typeof data === "object" ? data : DEFAULT_STATE;
  }, [stateResult]);
  const [busy, setBusy] = useState(null);
  const [error, setError] = useState(null);
  const applyToFleet = useCallback(
    async (paused) => {
      setError(null);
      if (!companyId) {
        return { okCount: 0, failCount: 0, companies: 0 };
      }
      try {
        const agents = await listAgentsForCompany(companyId);
        if (agents.length === 0) {
          return { okCount: 0, failCount: 0, companies: 1 };
        }
        let okCount = 0;
        let failCount = 0;
        await Promise.all(
          agents.map(async (a) => {
            try {
              const r = await postAgentPause(a.id, paused);
              if (r.ok) okCount++;
              else failCount++;
            } catch {
              failCount++;
            }
          })
        );
        return { okCount, failCount, companies: 1 };
      } catch (e) {
        setError(e?.message ?? "Failed to apply pause to fleet");
        return { okCount: 0, failCount: 0, companies: 0 };
      }
    },
    [companyId]
  );
  const onToggle = useCallback(async () => {
    if (busy) return;
    const next = !state.paused;
    setBusy(next ? "pause" : "run");
    try {
      await setPaused({
        companyId: companyId ?? void 0,
        paused: next,
        actorId: host.userId ?? "board",
        reason: "Top-bar Pause/Run toggle"
      });
      const summary = await applyToFleet(next);
      await refreshSummary({
        companyId: companyId ?? void 0,
        agents: summary.okCount,
        companies: summary.companies ?? 0
      });
    } catch (e) {
      setError(e?.message ?? "Failed to toggle pause state");
    } finally {
      setBusy(null);
    }
  }, [busy, state.paused, companyId, host.userId, setPaused, applyToFleet, refreshSummary]);
  useEffect(() => {
    if (!error) return;
    const t = setTimeout(() => setError(null), 5e3);
    return () => clearTimeout(t);
  }, [error]);
  const buttonLabel = state.paused ? "Run" : "Pause";
  const buttonColor = state.paused ? "#16a34a" : "#dc2626";
  const buttonHover = state.paused ? "#15803d" : "#b91c1c";
  const subtitle = state.paused && state.summary.agents > 0 ? `${state.summary.agents} agent${state.summary.agents === 1 ? "" : "s"} paused` : state.paused ? "Pause requested" : state.summary.agents > 0 ? `${state.summary.agents} agent${state.summary.agents === 1 ? "" : "s"} active` : "";
  return /* @__PURE__ */ jsxs(
    "div",
    {
      "data-testid": "pause-run-button",
      "data-paused": state.paused ? "true" : "false",
      style: {
        display: "inline-flex",
        alignItems: "center",
        gap: 8,
        marginLeft: 8
      },
      title: state.paused ? `System paused${state.pausedAt ? " " + formatRelative(state.pausedAt) : ""}${state.pausedBy ? " by " + state.pausedBy : ""}` : "System running \u2014 click Pause to halt agent dispatch (running agents continue their in-flight work)",
      children: [
        state.paused ? /* @__PURE__ */ jsxs(
          "span",
          {
            "aria-hidden": "true",
            style: {
              display: "inline-flex",
              alignItems: "center",
              gap: 4,
              padding: "2px 8px",
              borderRadius: 9999,
              background: "rgba(220, 38, 38, 0.12)",
              color: "#dc2626",
              fontSize: 11,
              fontWeight: 600,
              letterSpacing: 0.4,
              textTransform: "uppercase"
            },
            children: [
              /* @__PURE__ */ jsx("span", { style: {
                display: "inline-block",
                width: 6,
                height: 6,
                borderRadius: "50%",
                background: "#dc2626"
              } }),
              "Paused"
            ]
          }
        ) : null,
        /* @__PURE__ */ jsx(
          "button",
          {
            type: "button",
            onClick: onToggle,
            disabled: busy !== null,
            "aria-pressed": state.paused,
            style: {
              display: "inline-flex",
              alignItems: "center",
              gap: 6,
              padding: "4px 10px",
              borderRadius: 6,
              background: busy !== null ? "#9ca3af" : buttonColor,
              color: "#fff",
              border: "1px solid transparent",
              fontSize: 12,
              fontWeight: 600,
              cursor: busy !== null ? "wait" : "pointer",
              transition: "background 120ms ease",
              whiteSpace: "nowrap"
            },
            onMouseEnter: (e) => {
              if (busy === null) e.currentTarget.style.background = buttonHover;
            },
            onMouseLeave: (e) => {
              if (busy === null) e.currentTarget.style.background = buttonColor;
            },
            children: busy === "pause" ? "Pausing\u2026" : busy === "run" ? "Resuming\u2026" : buttonLabel
          }
        ),
        subtitle ? /* @__PURE__ */ jsx("span", { style: { fontSize: 11, color: "var(--muted-foreground, #6b7280)" }, children: subtitle }) : null,
        error ? /* @__PURE__ */ jsx(
          "span",
          {
            role: "alert",
            style: { fontSize: 11, color: "#dc2626", maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" },
            title: error,
            children: error
          }
        ) : null
      ]
    }
  );
}
var index_default = PauseRunButton;
export {
  PauseRunButton,
  index_default as default
};
