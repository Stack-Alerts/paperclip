 ...
 *
 * We DO forward PGPASSWORD if it's set in either process.env (host
 * env) or the repo's .env file — the script shells out to psql for
 * chain + approval data and the embedded Paperclip postgres uses
 * password auth, not trust. Without PGPASSWORD, every psql call exits
 * silently and the UI shows "0 chains / 0 approvals" even when data
 * exists. The .env lookup mirrors `loadDotenv()` above but only pulls
 * the keys we care about to avoid loading unrelated secrets.
 */
function buildChildEnv(repoPath?: string): NodeJS.ProcessEnv {
  ...
  // Pull PGPASSWORD from process.env first, then fall back to the
  // repo's .env. Don't override an explicit value with .env.
  let password = process.env.PGPASSWORD ?? process.env.PAPERCLIP_PGPASSWORD;
  if (!password && repoPath) {
    try {
      const raw = readFileSync(path.join(repoPath, ".env"), "utf8");
      for (const line of raw.split(/\r?\n/)) {
        ...
        if (key === "PGPASSWORD" || key === "PAPERCLIP_PGPASSWORD") {
      ...
      // .env missing or unreadable — child will skip psql queries.
    }
  }
  if (password) {
  ...
  await new Promise<void>((resolve) => {
    ...
    try {
      child = spawn(config.pythonPath, [scriptPath, ...args], {
        ...
        env: buildChildEnv(config.repoPath),
      });
    } catch (err) {
