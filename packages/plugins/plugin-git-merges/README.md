...
- A **Git Merges** link in the left sidebar's **Work** section,
  ...
  `order: 50`).
- A **Git Merges** page at `/:companyPrefix/git-merges` with five
  ...
  - **Blocked Chain** — connected components of the
    `issue_relations` graph, with leaves/roots highlighted and an
    "actionable blocks" summary.
  - **Awaiting Approval** — every pending board approval for the
    company (same source as the operator's Inbox), with chain context
    attached when the linked issue is a member of a blocked chain so
    the "⚡ unblocks N" badge lights up on impact-heavy items.
...
## Required env: Paperclip embedded-postgres password

The script fetches chain and approval data directly from the embedded
Paperclip postgres (port 54329) via `psql`. Paperclip's embedded
postgres uses **password** auth (`server/src/index.ts` — user
`paperclip`, password `paperclip`), not trust auth as older docs
implied. If `PGPASSWORD` (or `PAPERCLIP_PGPASSWORD`) is not set, psql
exits with `fe_sendauth: no password supplied` and the script silently
returns empty data — which surfaces in the UI as **"0 chains / 0
approvals"** even when the Inbox shows pending approvals and the DB
has block relations.

Add this line to the worktree's `.env` (the same file the script
already reads for Paperclip/GitHub credentials):

```sh
PGPASSWORD=paperclip
```

Or expose it to the worker process however the host environment is
configured (systemd unit, Docker `environment:`, shell rc, etc.).
The plugin worker's child env forwards `PGPASSWORD` to psql
unmodified.
