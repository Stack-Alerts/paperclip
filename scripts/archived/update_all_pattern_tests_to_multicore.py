"""
Update all pattern test scripts to use multicore processing
This will make them MUCH faster by using all CPU cores
"""
...
# Pattern mapping: test number -> (pattern_name, ClassName)
PATTERNS = {
    33: ('triple_top', 'TripleTopPattern'),
    34: ('triple_bottom', 'TripleBottomPattern'),
    35: ('head_and_shoulders', 'HeadAndShouldersPattern'),
    36: ('inverse_head_and_shoulders', 'InverseHeadAndShouldersPattern'),
    37: ('cup_and_handle', 'CupAndHandlePattern'),
    38: ('rounding_bottom', 'RoundingBottomPattern'),
    39: ('flag_pattern', 'FlagPattern'),
    40: ('pennant_pattern', 'PennantPattern'),
    41: ('symmetrical_triangle', 'SymmetricalTrianglePattern'),
    42: ('ascending_triangle', 'AscendingTrianglePattern'),
    43: ('descending_triangle', 'DescendingTrianglePattern'),
    44: ('falling_wedge', 'FallingWedgePattern'),
    45: ('rising_wedge', 'RisingWedgePattern'),
}


MULTICORE_TEMPLATE = '''"""
Walk-Forward Test for {pattern_display}
...
if __name__ == "__main__":
    from src.detectors.building_blocks.patterns.{pattern_file} import {class_name}
    ...
    if df is not None and len(df) > 0:
        ...
        test_block_walkforward_v2({class_name}, {{}}, "{pattern_name}", df)
    else:
...
def main():
    """Generate all multicore pattern test scripts"""
    
    base_path = Path(__file__).parent / 'walkforward_tests'
    
    print("🔧 Updating pattern test scripts to multicore processing...")
    print("="*80)
    
    for test_num, (pattern_name, class_name) in PATTERNS.items():
        # Convert snake_case to Title Case for display
        pattern_display = pattern_name.replace('_', ' ').title()
        
        # Pattern file is same as pattern name
        ...
        # Write file
        output_file = base_path / f'{test_num:02d}_test_{pattern_name}.py'
        with open(output_file, 'w') as f:
    ...
    print("="*80)
    print(f"✅ Updated {len(PATTERNS)} pattern test scripts to use multicore processing!")
