...
if [[ "$TIER" == "hourly" ]]; then
    ...
    # full re-download from gdrive). The snapshot creator stores every
    # snapshot in YYYY-MM-DD-HHMM format with a random HHMM (not the
    # historical -0000 convention) so we match any YYYY-MM-DD-HHMM in
    # the snapshots dir and prefer the oldest one that exists.
    # We deliberately pick the OLDEST available local daily, because the
    # rsync --compare-dest algorithm computes the diff between the
    # current snapshot and the chosen base — using the oldest valid base
    # maximizes delta size (fewer assumed-unchanged files) and is the
    # safest fallback when no dedicated daily snapshot exists locally.
    PARENT_ID=$(ls -1 "$SNAPSHOTS_DIR" 2>/dev/null \
        | /usr/bin/grep -E '^[0-9]{4}-[0-9]{2}-[0-9]{2}-[0-9]{4}$' \
        | sort \
        | head -1 || true)

    if [[ -z "$PARENT_ID" ]]; then
        log "  no local snapshot found; checking gdrive:$TIER_ROOT/daily/ ..."
