"""
Update remaining test scripts (51-67) to use multicore processing
Same approach as update_all_pattern_tests_to_multicore.py
"""

from pathlib import Path


# Test mapping: test number -> (module_path, ClassName, test_name)
TESTS = {
    51: ('elliott_wave.elliott_wave_count', 'ElliottWaveCount', 'elliott_wave_count'),
    52: ('elliott_wave.elliott_wave_oscillator', 'ElliottWaveOscillator', 'elliott_wave_oscillator'),
    53: ('wyckoff.wyckoff_accumulation', 'WyckoffAccumulation', 'wyckoff_accumulation'),
    54: ('wyckoff.wyckoff_distribution', 'WyckoffDistribution', 'wyckoff_distribution'),
    55: ('wyckoff.wyckoff_reaccumulation', 'WyckoffReaccumulation', 'wyckoff_reaccumulation'),
    56: ('fibonacci.fibonacci_retracements', 'FibonacciRetracements', 'fibonacci_retracements'),
    57: ('vwap.anchored_vwap', 'AnchoredVWAP', 'anchored_vwap'),
    58: ('ema.ema_crossover', 'EMACrossover', 'ema_crossover'),
    59: ('market_structure.market_depth', 'MarketDepth', 'market_depth'),
    60: ('order_flow.order_flow_imbalance', 'OrderFlowImbalance', 'order_flow_imbalance'),
    61: ('price_action.premium_discount_zones', 'PremiumDiscountZones', 'premium_discount_zones'),
    62: ('liquidity.range_liquidity', 'RangeLiquidity', 'range_liquidity'),
    63: ('market_structure.swing_points', 'SwingPoints', 'swing_points'),
    64: ('sessions.kill_zones', 'KillZones', 'kill_zones'),
    65: ('sessions.session_time', 'SessionTime', 'session_time'),
    66: ('sessions.us_settlement', 'USSettlement', 'us_settlement'),
    67: ('supply_demand.supply_demand_zones', 'SupplyDemandZones', 'supply_demand_zones'),
}
...
if __name__ == "__main__":
    ...
    if df is not None and len(df) > 0:
        ...
        test_block_walkforward_v2({class_name}, {{}}, "{test_name}", df)
    else:
...
def main():
    """Generate all multicore test scripts for tests 51-67"""
    
    base_path = Path(__file__).parent / 'walkforward_tests'
    
    print("🔧 Updating test scripts 51-67 to multicore processing...")
    print("="*80)
    
    for test_num, (module_path, class_name, test_name) in TESTS.items():
        # Display name from test_name
        display_name = test_name.replace('_', ' ').title()
        
        # Generate script content from template
        script_content = MULTICORE_TEMPLATE.format(
        ...
        # Write file
        output_file = base_path / f'{test_num:02d}_test_{test_name}.py'
        with open(output_file, 'w') as f:
            f.write(script_content)
        
        print(f"✅ Updated: {output_file.name} ({class_name})")
    
    print("="*80)
    print(f"✅ Updated {len(TESTS)} test scripts to use multicore processing!")
    print("\n🚀 These scripts will now use ALL CPU cores for much faster execution!")
