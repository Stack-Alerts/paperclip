"""
Walk-Forward Test for Ascending Triangle
...
if __name__ == "__main__":
    from src.detectors.building_blocks.patterns.ascending_triangle import AscendingTriangle
    
    print("Loading 180 days of BTC 15min data...")
    df = load_btc_data(days=180)
    
    if df is not None and len(df) > 0:
        block = AscendingTriangle()
        test_block_walkforward_v2(block, "ascending_triangle", df)
    else:
