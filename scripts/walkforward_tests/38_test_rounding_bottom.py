"""
Walk-Forward Test for Rounding Bottom
...
if __name__ == "__main__":
    from src.detectors.building_blocks.patterns.rounding_bottom import RoundingBottom
    
    print("Loading 180 days of BTC 15min data...")
    df = load_btc_data(days=180)
    
    if df is not None and len(df) > 0:
        block = RoundingBottom()
        test_block_walkforward_v2(block, "rounding_bottom", df)
    else:
