"""
Walk-Forward Test for Symmetrical Triangle
...
if __name__ == "__main__":
    from src.detectors.building_blocks.patterns.symmetrical_triangle import SymmetricalTriangle
    
    print("Loading 180 days of BTC 15min data...")
    df = load_btc_data(days=180)
    
    if df is not None and len(df) > 0:
        block = SymmetricalTriangle()
        test_block_walkforward_v2(block, "symmetrical_triangle", df)
    else:
