"""
Walk-Forward Test for Head And Shoulders
...
if __name__ == "__main__":
    from src.detectors.building_blocks.patterns.head_and_shoulders import HeadAndShoulders
    
    print("Loading 180 days of BTC 15min data...")
    df = load_btc_data(days=180)
    
    if df is not None and len(df) > 0:
        block = HeadAndShoulders()
        test_block_walkforward_v2(block, "head_and_shoulders", df)
    else:
