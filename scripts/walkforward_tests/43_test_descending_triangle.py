"""
Walk-Forward Test for Descending Triangle
...
if __name__ == "__main__":
    from src.detectors.building_blocks.patterns.descending_triangle import DescendingTriangle
    
    print("Loading 180 days of BTC 15min data...")
    df = load_btc_data(days=180)
    
    if df is not None and len(df) > 0:
        block = DescendingTriangle()
        test_block_walkforward_v2(block, "descending_triangle", df)
    else:
