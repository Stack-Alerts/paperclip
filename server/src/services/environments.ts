...
/**
 * Derive a stable 48-bit advisory-lock key for a string input. Postgres
 * advisory locks take bigint, so we truncate the sha256 to 12 hex chars
 * (48 bits) which fits in JS's safe integer range and gives ~2^48 distinct
 * keys — collisions across unrelated callers are effectively impossible.
 */
function computeAdvisoryLockKey(input: string): number {
...
export function environmentService(db: Db) {
  return {
    ...
    ensureKubernetesEnvironment: async (
      ...
      // on driver='local' only, so two simultaneous callers (e.g. concurrent
      // heartbeats lazily provisioning a new company) can both see existing=null
      // and both insert. Serialize per {company, kind} with a Postgres
      // advisory lock held for the duration of the transaction. Callers that
      // arrive while the lock is held block on the lock acquisition, then
      // re-check existing under the lock and converge on the row the holder
      // inserted. The lock is auto-released on commit/rollback.
      const lockKey = computeAdvisoryLockKey(
        `paperclip:env:managed-k8s:${companyId}`,
      );
