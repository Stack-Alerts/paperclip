...
class TestStrategyInfoPanelGettersSetters:
    ...
    def test_required_signals_color_coding_zero(self, panel):
        """Test color coding for 0 required signals (uses text_disabled color)."""
        panel.set_required_signals(0)
        # Production uses get_color('text_disabled') = #E8EAED for zero
        assert "#E8EAED" in panel.required_signals_label.styleSheet()

    def test_required_signals_color_coding_nonzero(self, panel):
        """Test color coding for non-zero required signals (uses success color)."""
        panel.set_required_signals(3)
        # Production uses get_color('success') = #10B981 for all non-zero values
        assert "#10B981" in panel.required_signals_label.styleSheet()

    def test_required_signals_color_coding_medium(self, panel):
        """Test color coding for medium count required signals (still uses success color)."""
        panel.set_required_signals(8)
        # Production uses same success color regardless of count threshold
        assert "#10B981" in panel.required_signals_label.styleSheet()

    def test_required_signals_color_coding_high(self, panel):
        """Test color coding for high required signals count (still uses success color)."""
...
class TestStrategyInfoPanelOrchestrator:
    ...
    def test_create_strategy_in_orchestrator_success(self, panel, orchestrator):
        ...
        # Verify return value and that orchestrator was called
    ...
    def test_create_strategy_in_orchestrator_no_name(self, panel, orchestrator):
        ...
        # Reset mock to allow clean assertion
        orchestrator.create_strategy = Mock()
...
class TestStrategyInfoPanelAutoUpdate:
    ...
    def test_update_description_from_config_with_blocks(self, panel, orchestrator):
        """Test generating description from blocks when description exists."""
        # Setup mock config_engine so the description path is taken
        mock_signal1 = Mock()
        mock_signal1.name = "SIGNAL_1"
        mock_signal1.timing_constraint = None
        mock_signal1.recheck_config = None

        mock_block1 = Mock()
        mock_block1.name = "Block_1"
        mock_block1.signals = [mock_signal1]
        ...
        # Mock config_engine with generate_description
        mock_config_engine = Mock()
        mock_config_engine.generate_description = Mock(return_value="Generated description with Block_1")

        orchestrator.get_current_config = Mock(return_value=mock_config)
        orchestrator.config_engine = mock_config_engine
    ...
    def test_update_required_signals_from_config_with_and_blocks(self, panel, orchestrator):
        """Test calculating required signals from AND blocks."""
        # Setup - 2 signals in AND block = 2 required signals
        mock_signal1 = Mock()
        mock_signal2 = Mock()
        ...
        # Verify - 2 signals in AND block = 2 required
        assert panel.get_required_signals() == 2

    def test_update_required_signals_calculates_from_blocks(self, panel, orchestrator):
        """Test calculating required signals from multiple blocks."""
        # Setup - 2 AND signals in AND block, 1 OR block (not counted as required)
        mock_signal1 = Mock()
        ...
        # Verify - only AND block signals counted = 2
        # (OR block signals are optional, not required)
        assert panel.get_required_signals() == 2
    ...
    def test_refresh_from_orchestrator(self, panel, orchestrator):
        ...
        mock_config.strategy_type = "Bullish"
        orchestrator.get_current_config = Mock(return_value=mock_config)

        panel.set_strategy_name("Test")  # Set name to verify it persists

        # Execute
        panel.refresh_from_orchestrator()

        # Verify refresh ran without errors
        # When blocks is empty, description shows "No blocks added yet..."
        assert "No blocks" in panel.get_description()
        # Required signals defaults to 0 when no AND blocks
        assert panel.get_required_signals() == 0


class TestStrategyInfoPanelStatusUpdates:
    """Test suite for status indicator behavior via signals."""

    def test_name_change_emits_signal(self, panel):
        """Test that changing name emits strategy_name_changed signal."""
        spy = QSignalSpy(panel.strategy_name_changed)
        panel.name_input.setText("New_Strategy")
        assert len(spy) > 0
        assert "New_Strategy" in spy[-1][0]

    def test_clearing_name_emits_empty_signal(self, panel):
        """Test that clearing name emits empty string signal."""
        # Set a name first
        panel.name_input.setText("Test_Strategy")
    ...
    def test_strategy_type_change_emits_signal(self, panel):
        """Test that strategy type change emits strategy_type_changed signal."""
        # Ensure we start in bullish state
        panel.set_strategy_type("Bullish")
