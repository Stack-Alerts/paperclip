...
class TestStartingCapitalValidation:
    ...
    def test_default_value(self, panel):
        ...
        assert panel.capital_input.text() == '10000'
    ...
    def test_valid_amounts(self, panel):
        ...
        for amount in valid_amounts:
            ...
            assert '✓' in panel.capital_validation.text()
    ...
    def test_minimum_boundary(self, panel):
        ...
        assert '✗' in panel.capital_validation.text()
        assert 'must be >= $500' in panel.capital_validation.text()
    ...
    def test_maximum_boundary(self, panel):
        ...
        assert '✗' in panel.capital_validation.text()
        assert 'must be <= $1000000' in panel.capital_validation.text()
    
    def test_invalid_input(self, panel):
        """Test invalid input handling"""
        invalid_inputs = ['abc', '', '-100', '10.5.5', 'invalid']
    ...
    def test_decimal_amounts(self, panel):
        ...
        # Should be valid since Money type handles decimals
        assert panel.get_starting_capital() == Money('10000.50', usd)
...
class TestStartingCapitalUI:
    ...
    def test_quick_set_buttons(self, panel, qapp):
        ...
        usd = Currency.from_str('USD')
        # Quick-set buttons should update the input field
        # Simulating button click by directly setting value
        expected_values = [500, 1000, 2500, 5000, 10000, 25000, 50000]
        
        for value in expected_values:
            panel.capital_input.setText(str(value))
            qapp.processEvents()
            assert panel.get_starting_capital() == Money(str(value), usd)
    
    def test_validation_ui_feedback(self, panel, qapp):
        """Test validation provides UI feedback"""
        # Valid input: green check
        panel.set_starting_capital('10000')
        qapp.processEvents()
        assert '✓' in panel.capital_validation.text()
        assert 'Valid' in panel.capital_validation.text()
        
        # Invalid input: red X with error
        panel.set_starting_capital('100')
        qapp.processEvents()
        assert '✗' in panel.capital_validation.text()
        assert 'must be' in panel.capital_validation.text()
...
class TestStartingCapitalIntegration:
    ...
    def test_setter_method(self, panel):
        ...
        assert panel.capital_input.text() == '75000'
        assert panel.get_starting_capital() == Money('75000', usd)
    
    def test_money_type_precision(self, panel):
        """Test Money type preserves decimal precision"""
        panel.set_starting_capital('12345.67')
        capital = panel.get_starting_capital()
        assert float(capital.as_decimal()) == 12345.67
