"""
Regression tests for BTCAAAAA-25737: Touch Index bug-close ingestion worker.

Issue: https://app.paperclip.ing/BTCAAAAA/issues/BTCAAAAA-25737
Component: src/touch_index/__main__.py, src/touch_index/bug_worker.py

Root cause: The bug-close ingestion worker's polling path unconditionally
transitioned all issues to done, including non-done issues. The fix adds a
guard that only transitions issues already in "done" status (or unknown/None).
...
pytestmark = [
    pytest.mark.bug("BTCAAAAA-25737"),
