"""Unit tests for scripts/run_touch_index_fr_worker.py main() entry point.
...
_CLEAN_ARGV = ["run_touch_index_fr_worker.py"]


class TestFrRunnerMain:
    def test_db_health_check_failure_exits(self, monkeypatch):
        ...
        with (
            ...
            patch("run_touch_index_fr_worker.health_check", return_value=False),
    ...
    def test_no_issues_returns_early(self, monkeypatch):
        """When no FDR issues are returned, run_fr_worker is never called."""
        monkeypatch.setattr(sys, "argv", _CLEAN_ARGV)
        with (
            patch("run_touch_index_fr_worker.get_engine", return_value=_make_engine()),
            patch("run_touch_index_fr_worker.health_check", return_value=True),
            patch("run_touch_index_fr_worker.get_fdr_issues", return_value=[]),
            patch("run_touch_index_fr_worker.run_fr_worker") as mock_worker,
        ):
            main()

        mock_worker.assert_not_called()

    def test_issues_found_calls_run_fr_worker(self, monkeypatch):
        """When issues exist, run_fr_worker is called with engine and the issues."""
        monkeypatch.setattr(sys, "argv", _CLEAN_ARGV)
    ...
    def test_cutoff_is_within_lookback_window(self, monkeypatch):
        ...
        def _capture_cutoff(updated_after=None):
            captured["cutoff"] = updated_after
            return []
        ...
        with (
            ...
            patch("run_touch_index_fr_worker.health_check", return_value=True),
            patch(
                "run_touch_index_fr_worker.get_fdr_issues", side_effect=_capture_cutoff
            ),
        ):
    ...
    def test_custom_lookback_minutes_arg(self, monkeypatch):
        ...
        monkeypatch.setattr(
            sys, "argv", ["run_touch_index_fr_worker.py", "--lookback-minutes", "60"]
        )
        captured = {}

        def _capture(updated_after=None):
            captured["cutoff"] = updated_after
            return []
        ...
        with (
            patch("run_touch_index_fr_worker.get_engine", return_value=_make_engine()),
            patch("run_touch_index_fr_worker.health_check", return_value=True),
            patch("run_touch_index_fr_worker.get_fdr_issues", side_effect=_capture),
        ):
            before = datetime.now(timezone.utc)
            main()
            after = datetime.now(timezone.utc)
...
class TestFrRunnerIssueId:
    def test_issue_id_calls_process_fr_issue(self, monkeypatch):
        """When --issue-id is provided, process_fr_issue is called instead of the polling path."""
        monkeypatch.setattr(
    ...
    def test_issue_id_not_found_logs_and_returns(self, monkeypatch, caplog):
        """When process_fr_issue returns None, a message is logged and we return cleanly."""
        import logging

        monkeypatch.setattr(
            sys, "argv", ["run_touch_index_fr_worker.py", "--issue-id", "missing-uuid"]
        )
        ...
        with (
            ...
            patch("run_touch_index_fr_worker.health_check", return_value=True),
            patch(
                "run_touch_index_fr_worker.process_fr_issue", return_value=None
            ),
...
class TestFrRunnerValidate:
    def test_validate_calls_validation_after_ingestion(self, monkeypatch, caplog):
        ...
        monkeypatch.setattr(
            sys, "argv", ["run_touch_index_fr_worker.py", "--validate"]
        )
        issues = [
            {"id": "id-1", "identifier": "BTCAAAAA-101", "description": ""},
        ]
    ...
    def test_validate_with_no_issues_failure_exits(self, monkeypatch):
        ...
        with (
            ...
            patch("run_touch_index_fr_worker.health_check", return_value=True),
            patch("run_touch_index_fr_worker.get_fdr_issues", return_value=[]),
            patch("run_touch_index_fr_worker.run_fr_worker"),
            patch("run_touch_index_fr_worker._run_validation", return_value=3),
        ):
            with pytest.raises(SystemExit) as exc_info:
                main()
    ...
    def test_validate_not_called_without_flag(self, monkeypatch):
        ...
        with (
            ...
            patch("run_touch_index_fr_worker.run_fr_worker", return_value=results),
            patch("run_touch_index_fr_worker.transition_issue_status"),
            patch("run_touch_index_fr_worker._run_validation") as mock_val,
        ):
            main()
