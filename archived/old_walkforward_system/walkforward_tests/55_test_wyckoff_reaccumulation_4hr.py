"""
Walk-Forward Test for Wyckoff Reaccumulation - 4HR TIMEFRAME
...
HIGHER TIMEFRAME - Wyckoff works better on 4hr+ (not 15min micro-ranges)
"""
...
def load_btc_data_4hr(days: int = 180) -> pd.DataFrame:
    """
    Load BTC 4HR data (converted from 15min or 1hr data)
    ...
    if data_path_1hr.exists():
        print("Loading 1HR data and resampling to 4HR...")
        df = pd.read_csv(data_path_1hr)
    elif data_path_15min.exists():
        print("Loading 15MIN data and resampling to 4HR...")
        df = pd.read_csv(data_path_15min)
    else:
        raise FileNotFoundError("Need either 1HR or 15MIN data to create 4HR")
    ...
    print(f"✅ 4HR data created: {len(df_4hr)} bars")
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("WYCKOFF REACCUMULATION - 4HR TIMEFRAME TEST")
    ...
    try:
        ...
        print("\n📊 Wyckoff Reaccumulation on 4HR: Should have EXCELLENT distribution")
        print("   - Very selective (like 4HR Accumulation/Distribution)")
        print("   - Only true reaccumulation phases")
        print("   - Spring/Breakout extremely rare")
        print("   - Target: 10-20% REACCUM, 80-90% NO_REACCUM, very rare Spring/Breakout")
        print("="*80)
