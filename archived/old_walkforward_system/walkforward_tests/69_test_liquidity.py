"""
Walk-Forward Test for Liquidity
...
def test_block_walkforward_v2(block_class, block_kwargs, block_name: str, df_full: pd.DataFrame):
    ...
    active_signals = [s for s in signals if s not in ['NEUTRAL', 'INSUFFICIENT_DATA', 'ERROR', 'NO_IB']]
    ...
    if has_event_tracking:
        ...
        print(f"   Continuing state: {len(active_signals) - new_event_count}")
...
if __name__ == "__main__":
    from src.detectors.building_blocks.market_structure.liquidity import Liquidity
