"""
Walk-Forward Test for Elliott Wave Count
...
def load_btc_data(timeframe: str, days: int = 180) -> pd.DataFrame:
    """
    Load BTC data for specified timeframe (INSTITUTIONAL GRADE)
    
    Args:
        timeframe: '15min', '4h', or '1d'
    ...
    timeframe_files = {
        '15min': 'BTC_USDT_PERP_15m.csv',
        '4h': 'BTC_USDT_PERP_4h.csv',
        '1d': 'BTC_USDT_PERP_1d.csv'
    }
    
    if timeframe not in timeframe_files:
        raise ValueError(f"Unsupported timeframe: {timeframe}. Use '15min', '4h', or '1d'")
    
    data_path = Path(__file__).parent.parent.parent / 'data' / 'raw' / timeframe_files[timeframe]
...
def process_chunk(args):
    ...
    Args: tuple of (chunk_indices, df_full_dict, block_class, block_kwargs, df_4h_dict, df_1d_dict)
    Returns: list of (result_dict, error_message or None) tuples
    """
    chunk_indices, df_full_dict, block_class, block_kwargs, df_4h_dict, df_1d_dict = args
    
    # Reconstruct dataframes once per chunk
    df_full = pd.DataFrame(df_full_dict)
    df_4h = pd.DataFrame(df_4h_dict) if df_4h_dict else None
    df_1d = pd.DataFrame(df_1d_dict) if df_1d_dict else None
    ...
    for i in chunk_indices:
        try:
            ...
            # Analyze with HTF data if available
            if df_4h is not None and df_1d is not None:
                result = block.analyze(hist_df, df_4h=df_4h, df_1d=df_1d)
            else:
...
def test_block_walkforward_v2(block_class, block_kwargs, block_name: str, df_full: pd.DataFrame, 
                              df_4h: pd.DataFrame = None, df_1d: pd.DataFrame = None):
    """
    ...
    if df_4h is not None:
        print(f"4H Dataset: {len(df_4h)} bars")
    if df_1d is not None:
        print(f"Daily Dataset: {len(df_1d)} bars")
    ...
    # Convert dataframes to dict once for all workers
    df_full_dict = df_full.to_dict('list')
    df_4h_dict = df_4h.to_dict('list') if df_4h is not None else None
    df_1d_dict = df_1d.to_dict('list') if df_1d is not None else None
    
    # Prepare work items (each worker gets a chunk)
    work_items = [(chunk, df_full_dict, block_class, block_kwargs, df_4h_dict, df_1d_dict) for chunk in chunks]
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("INSTITUTIONAL GRADE DATA LOADING")
    print("="*80)
    
    try:
        # Load 4H data for MTF
        print("Loading 180 days of BTC 4H data...")
        df_4h = load_btc_data('4h', days=180)
        print(f"✅ 4H data loaded: {len(df_4h)} bars")
        
        # Load Daily data for MTF
        print("Loading 180 days of BTC Daily data...")
        df_1d = load_btc_data('1d', days=180)
        print(f"✅ Daily data loaded: {len(df_1d)} bars")
        
        print("\n📊 MTF Elliott Wave: Uses Daily + 4H (HTF focus)")
        print("   - Daily: PRIMARY context (most significant)")
        print("   - 4H: INTERMEDIATE confirmation")
        print("   - Result: 70-95% confidence, +30-75 booster")
        print("="*80)
        
        # Test with MTF data (Daily + 4H only)
        test_block_walkforward_v2(
            ElliottWaveCount, 
            {'use_mtf': True}, 
            "elliott_wave_count", 
            df_4h,  # Use 4H as base for iteration  
            df_4h=df_4h,
            df_1d=df_1d
        )
        ...
        print("   - data/raw/BTC_USDT_PERP_1d.csv")
    except Exception as e:
        print(f"\n❌ Error: {e}")
