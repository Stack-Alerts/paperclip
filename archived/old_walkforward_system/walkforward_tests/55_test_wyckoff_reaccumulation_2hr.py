"""
Walk-Forward Test for Wyckoff Reaccumulation - 2HR TIMEFRAME
...
HIGHER TIMEFRAME - Wyckoff works better on 2hr+ (not 15min micro-ranges)
"""
...
def load_btc_data_2hr(days: int = 180) -> pd.DataFrame:
    """
    Load BTC 2HR data (converted from 15min or 1hr data)
    ...
    """
    # Try to load 1hr data and resample to 2hr
    ...
    if data_path_1hr.exists():
        print("Loading 1HR data and resampling to 2HR...")
        df = pd.read_csv(data_path_1hr)
    elif data_path_15min.exists():
        print("Loading 15MIN data and resampling to 2HR...")
        df = pd.read_csv(data_path_15min)
    else:
        raise FileNotFoundError("Need either 1HR or 15MIN data to create 2HR")
    ...
    print(f"✅ 2HR data created: {len(df_2hr)} bars")
...
def test_block_walkforward_v2_2hr(block_class, block_kwargs, block_name: str, df_full: pd.DataFrame):
    """
    Walk-forward test using V2 methodology with MULTICORE PROCESSING - 2HR TIMEFRAME
    """
    
    print("="*80)
    print(f"🔬 WALK-FORWARD TEST V2 (MULTICORE) - 2HR TIMEFRAME: {block_name}")
    print("="*80)
    ...
    active_signals = [s for s in signals if s not in ['NEUTRAL', 'INSUFFICIENT_DATA', 'ERROR', 'NO_REACCUMULATION']]
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("WYCKOFF REACCUMULATION - 2HR TIMEFRAME TEST")
    print("Hypothesis: Wyckoff works MUCH better on higher timeframes!")
    print("="*80)
    
    try:
        # Load 2HR data
        print("Loading 180 days of BTC 2HR data...")
        df_2hr = load_btc_data_2hr(days=180)
        
        print("\n📊 Wyckoff Reaccumulation on 2HR: Should have BETTER distribution")
        print("   - Less micro-ranging than 15min")
        print("   - True reaccumulation phases visible")
        print("   - Spring/Breakout events more likely")
        print("   - Target: 25-35% REACCUM, 65-75% NO_REACCUM, rare Spring/Breakout")
        print("="*80)
        
        # Test with optimized settings
        test_block_walkforward_v2_2hr(
