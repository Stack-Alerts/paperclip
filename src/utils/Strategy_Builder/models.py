...
class StrategyConfiguration(BaseModel):
    ...
    def validate_strategy(self):
        ...
        # Auto-fix strategy name to include number prefix
        prefix = f"strategy_{self.strategy_number:02d}_"
        if not self.strategy_name.startswith(prefix):
            name_no_spaces = self.strategy_name.replace(' ', '_')
            self.strategy_name = f"{prefix}{name_no_spaces}"
