...
class StrategyGenerator:
    ...
    def __init__(self, template_dir: Optional[Path] = None):
        ...
        # Load templates (lazy - will be loaded on first use)
        self._strategy_template = None
        self._test_template = None
        self._optimizer_template = None
        
    @property
    def strategy_template(self):
        if self._strategy_template is None:
            self._strategy_template = self.env.get_template('strategy_template.py.j2')
        return self._strategy_template
    
    @property
    def test_template(self):
        if self._test_template is None:
            self._test_template = self.env.get_template('test_template.py.j2')
        return self._test_template
    
    @property
    def optimizer_template(self):
        if self._optimizer_template is None:
            self._optimizer_template = self.env.get_template('optimizer_config.yaml.j2')
        return self._optimizer_template
