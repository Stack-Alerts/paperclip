...
Purpose: Trend identification and reversal detection using 55-period EMA with PVSRA/TBD vector detection
"""
"""
Building Block Classification: SIGNAL BLOCK
Mode: EVENT-DRIVEN
Purpose: Price breaks through 55 EMA, selective signals
...
@register_block(
    ...
    signal_tiers={
        ...
        'BULLISH_CLIMAX': {
            'base_points': 22,
            'formula': 'scaled',
            'description': 'Bullish climax vector - crossed above 55 EMA with 160%+ volume (high conviction)'
        },
        'BEARISH_CLIMAX': {
            'base_points': 22,
            'formula': 'scaled',
            'description': 'Bearish climax vector - crossed below 55 EMA with 160%+ volume (high conviction)'
        },
        'BULLISH_PSEUDO': {
            'base_points': 20,
            'formula': 'scaled',
    ...
    description='EMA 55 Vector - Detects high-volume breaks through 55 EMA with PVSRA/TBD methodology (differentiated thresholds)',
    tags=['moving_averages', 'ema', 'vector', 'pvsra', 'volume', 'signal_block']
)
class EMA55VectorBreak:
    """
    55 EMA Vector Break - Trend Following Indicator with PVSRA/TBD Implementation
    
    The 55 EMA provides a middle ground between 50 and 200 EMA,
    offering slightly more lag than 50 EMA while being more responsive than 200 EMA.
    ...
    Signals:
    - Vector Break Up: Price breaks above 55 EMA with PVSRA vector
    - Vector Break Down: Price breaks below 55 EMA with PVSRA vector
    ...
    def __init__(self, 
                 period: int = 45,  # OPTIMIZED: 45 performs best across EMA vectors
                 timeframe: str = '15min',
                 ...
                 **kwargs):
        """
        Initialize 55 EMA Vector block with tunable parameters
    ...
    def analyze(self, df: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        ...
        if 'volume' in df.columns and len(df) >= 11:
            ...
            # PVSRA Tier 2: Climax Vector (160%+ - DIFFERENTIATED from EMA 50 for tiered system)
            if current_volume >= (vol_ma_10 * 1.6):
                is_vector_candle = True
                vector_tier = "CLIMAX_GREEN" if is_bullish_candle else "CLIMAX_RED"
                
            # PVSRA Tier 1: Pseudo Vector (120%+ - DIFFERENTIATED from EMA 50 for tiered system)
            elif current_volume >= (vol_ma_10 * 1.2):
                is_vector_candle = True
        ...
        # Emit GRANULAR signals for advanced users
        if crossed_above and is_vector_candle:
            # Bullish vector break
            
            # CLIMAX vectors (160%+): Always take
            if "CLIMAX" in vector_tier:
                signal = 'BULLISH_CLIMAX'  # GRANULAR
                confidence = 95
                confluence_factors.append(f'⚡ CLIMAX VECTOR: {vector_tier} (160%+ volume)')
                
                if slope in ['RISING', 'STRONG_RISING']:
                    confidence += 5
                    confluence_factors.append('✅ 55 EMA slope confirming uptrend')
                    
                confluence_factors.append('📈 BULLISH CLIMAX: Climax vector crossed above 55 EMA')
            
            # PSEUDO vectors (120%+): Require slope confirmation
            elif "PSEUDO" in vector_tier:
                if slope in ['RISING', 'STRONG_RISING']:
                    signal = 'BULLISH_PSEUDO'  # GRANULAR
                    confidence = 90
                    confluence_factors.append(f'📊 PSEUDO VECTOR: {vector_tier} (120%+ volume)')
                    confluence_factors.append('✅ 55 EMA slope confirming uptrend - CONFIRMED')
                    confluence_factors.append('📈 BULLISH PSEUDO: Confirmed pseudo vector crossed above 55 EMA')
            ...
            if "CLIMAX" in vector_tier:
                ...
                if slope in ['FALLING', 'STRONG_FALLING']:
                    confidence += 5
                    confluence_factors.append('✅ 55 EMA slope confirming downtrend')
                    
                confluence_factors.append('📉 BEARISH CLIMAX: Climax vector crossed below 55 EMA')
            ...
            # PSEUDO vectors (120%+): Require slope confirmation
                ...
                if slope in ['FALLING', 'STRONG_FALLING']:
                    ...
                    confidence = 90
                    confluence_factors.append(f'📊 PSEUDO VECTOR: {vector_tier} (120%+ volume)')
                    confluence_factors.append('✅ 55 EMA slope confirming downtrend - CONFIRMED')
                    confluence_factors.append('📉 BEARISH PSEUDO: Confirmed pseudo vector crossed below 55 EMA')
        
        # Status information  
        if signal == 'NEUTRAL':
            if current_position == 'ABOVE_EMA':
                confluence_factors.append('Price above 55 EMA - uptrend bias')
            elif current_position == 'BELOW_EMA':
                confluence_factors.append('Price below 55 EMA - downtrend bias')
            
            confluence_factors.append(f'Distance from 55 EMA: {distance_pct:+.2f}% ({distance_class})')
            confluence_factors.append('No cross event - holding position')
        
        # Add slope context
        if slope == 'RISING':
            confluence_factors.append('55 EMA trending up')
        elif slope == 'FALLING':
            confluence_factors.append('55 EMA trending down')
        elif slope == 'FLAT':
